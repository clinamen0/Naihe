...
3. Switch language with the **EN / 中 / RU / فا** buttons in the top bar
...
### Why You Should Rewrite the Cipher — and Add Anti-Reverse-Engineering

**We strongly recommend that every deployment rewrites `cipher.rs` with a custom encryption scheme**, and adds anti-reverse-engineering measures (obfuscation, anti-debugging, integrity checks) to the compiled binary.

#### How the defense layers work together

Even if an adversary obtains the encryption key (e.g. through coercion or compromise), they still cannot decrypt the intercepted messages — **unless they also reverse-engineer the binary to extract the exact algorithm and parameters used by your specific build**. If the adversary cannot obtain the passphrase to enter the application, and cannot reverse-engineer the compiled binary, they are left holding a key with no idea what lock it fits.

```
Passphrase gate  →  protects access to the running application
Custom cipher    →  algorithm known only to your build
Anti-RE measures →  raises the cost of extracting the cipher from the binary
                 =  a leaked key alone is not enough to decrypt
```

#### An honest caveat

You should assume that **any binary delivered to an adversary can eventually be understood, modified, and bypassed** — given enough time, skill, and motivation. Anti-reverse-engineering does not make this impossible. What it does is:

- **Raise the analysis threshold** — casual inspection becomes insufficient
- **Extend the time required** — buying time for users to rotate keys or migrate
- **Increase the cost of scaled exploitation** — breaking one custom build does not break all others

The value is not in creating an unbreakable wall. It is in making the wall expensive enough that breaking it, at scale, for every unique deployment, becomes impractical.

#### In practice

- Rewrite `cipher.rs` with your own algorithm, key derivation, and wire format
- Add binary obfuscation (string encryption, control flow flattening, etc.)
- Strengthen `shield.rs` with more aggressive anti-debugging techniques
- Change the gate passphrase and salt
- **Do not publish your modified source code**

Every fork with a different cipher is a different lock. There is no master key that opens them all — and that is the point.
